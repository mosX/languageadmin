<div id="top_menu">
    ...
    <div class="page_menu">
        <div class="preset">
            <div class='title_button'>Жанры</div>
        </div>
        ...
        <div class="actions">
            ...
            <div data-toggle="modal" data-target="#addGenreModal" class="button add_deal">+ НОВЫЙ ЖАНР</div>
        </div>
    </div>
</div>
...
<script>
    app.controller('genresEditModalCtrl', ['$scope','$http',function($scope,$http,$userinfo){
        ...
        $scope.submit = function(event){
            $scope.form.filename = $('#genresEditModal input[name=filename]').val();
                ...
                <form action="" method="POST" ng-submit="submit($event)">
                    ...
                    <div class="form-group">
                        <div class="row">                            
                            ...
                            <div class="col-sm-8">
                                <input class="form-control" type="text" ng-model="form.sequence">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="uploadFileBtn">Загрузить
                                    <iframe id="hiddenIframeUpload" src="/genres/loadeditlogo/"></iframe>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <img class="preview_logo" style="display:block; margin:auto;" src="<?=$this->config->assets_url?>/genres/thumb_{{filename}}">
                            </div>
                        </div>
                    </div>
                    
                    <script>
                        function editImage(filepath,filename){
                            console.log(filename);
                            if($('#genresEditModal').css('display') != 'none'){
                                $('#genresEditModal input[name=filename]').val(filename);
                                $('#genresEditModal .preview_logo').attr('src',filepath);
                            }else if($('#editChannelModal').css('display') != 'none'){
                                $('#genresEditModal input[name=logo]').val(filename);
                                $('#genresEditModal .preview_logo').attr('src',filepath);
                            }
                        }
...
<script>
    app.controller('addGenreModalCtrl', ['$scope','$http',function($scope,$http,$userinfo){        
        $scope.form = {};

        $scope.submit = function(event){            
            $scope.form.filename = $('#addGenreModal input[name=filename]').val();
...
<style>
    #addGenreModal .preview img{
        max-width:100px;
        max-height:100px;
    }
</style>

<div ng-controller="addGenreModalCtrl" class="modal fade" id="addGenreModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button class="close" data-dismiss="modal">×</button>
                ...
                <form action="" method="POST" ng-submit="submit($event)">
                        ...
                        <div class="row">                            
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-sm-8">                                        
                                        <input type="text" class="form-control" name="sequence" ng-model="form.sequence">                                        
                                    </div>
                                </div>                                
                            </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="uploadFileBtn">Загрузить
                                    <iframe id="hiddenIframeUpload" src="/genres/loadaddlogo/"></iframe>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <img class="preview_logo" style="display:block; margin:auto;" src="<?=$this->config->assets_url?>/genres/thumb_{{filename}}">
                    ...
                    <script>
                        function addImage(filepath,filename){
                            console.log(filename);
                            if($('#addGenreModal').css('display') != 'none'){
                                $('#addGenreModal input[name=filename]').val(filename);
                                $('#addGenreModal .preview_logo').attr('src',filepath);
                            }
                            /*}else if($('#addGenreModal').css('display') != 'none'){
                                $('#addGenreModal input[name=logo]').val(filename);
                                $('#addGenreModal .preview_logo').attr('src',filepath);
                            }*/
                        }
